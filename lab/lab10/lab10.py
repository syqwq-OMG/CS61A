...
def floor_div(args):
    ...
    l_op = calc_eval(args.first)
    r_op = calc_eval(args.rest.first)

    return floor_div(Pair(l_op // r_op, args.rest.rest))
...
def addition(args):
    ...
    return addition(Pair(l_op + r_op, args.rest.rest))
...
def eval_and(expressions):
    ...
    return False if expressions is scheme_f else True
...
def eval_define(expressions):
    ...
    if val.rest is nil:
        ...
        # print("DEBUG:{} <- {} ".format(symbol, bindings[symbol]))
    else:
