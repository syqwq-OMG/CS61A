...
def do_define_form(expressions, env):
    ...
    # print("DEBUG:", expressions.__repr__())

    signature = expressions.first
    if scheme_symbolp(signature):
        # assigning a name to a value e.g. (define x (+ 1 2))z
        validate_form(
            expressions, 2, 2
        ...
        2
        scm> (define x (+ 7 3))
        DEBUG: Pair(Pair('+', Pair(7, Pair(3, nil))), nil)
        x
        scm> x
        (+ 7 3)
        """
        env.define(signature, scheme_eval(body.first, env))
        return signature
        # END PROBLEM 4
...
def do_or_form(expressions, env):
    ...
    if is_scheme_true((cur := scheme_eval(expressions.first, env))):
        return cur
