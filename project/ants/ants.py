...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not self.is_damage_doubled:
            self.damage, self.is_damage_doubled = self.damage * 2, True
        # END Problem 12

    def restore(self):
        if self.is_damage_doubled:
            self.damage, self.is_damage_doubled = self.damage // 2, False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_ptr, distance = self.place, 0
        while (not place_ptr.is_hive) and place_ptr.entrance:
            place_bees = place_ptr.bees
            if (
                len(place_bees) != 0
                and self.lower_bound <= distance <= self.upper_bound
            ):
                return random_bee(place_bees)

            place_ptr, distance = place_ptr.entrance, distance + 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # give damage if fireant died
        if self.health <= amount:
            for bee in self.place.bees[:]:
                Insect.reduce_health(bee, self.damage)
        # reflect damage to the bees
        for bee in self.place.bees[:]:
            Insect.reduce_health(bee, amount)
        # handling receive damage
...
# The HungryAnt Class
class HungryAnt(Ant):
    """Implement the HungryAnt, which will select a random Bee from its place and deal damage to the Bee equal to the Bee's health, eating it whole. After eating a Bee, a HungryAnt must spend 3 turns chewing before being able to eat again. While the HungryAnt is chewing, it is not able to eat (deal damage to) any Bees. If there is no bee in its place available to eat, the HungryAnt will do nothing."""
    ...
    def action(self, gamestate):
        ...
        if self.cooldown > 0:
            ...
            if len(cur_bees := self.place.bees[:]) != 0:
                self.cooldown = self.chew_cooldown
                self.chewing_bee = random_bee(cur_bees)
                Insect.reduce_health(self.chewing_bee, self.chewing_bee.health)
...
class ScubaThrower(ThrowerAnt):
    """Implement the ScubaThrower, which is a subclass of ThrowerAnt that is more costly and waterproof, but otherwise identical to its base class. A ScubaThrower should not lose its health when placed in Water."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        place_ptr = self.place.exit
        while place_ptr != None:
            place_ant = place_ptr.ant

            if place_ant != None:
                if isinstance(place_ant, ContainerAnt) and place_ant.ant_contained:
                    place_ant.ant_contained.double()
                place_ptr.ant.double()

            place_ptr = place_ptr.exit
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            ...
            while place_ptr:
                ...
                if place_ant:
                    if isinstance(place_ant, ContainerAnt) and place_ant.ant_contained:
                        place_ant.ant_contained.restore()
                    place_ptr.ant.restore()
                place_ptr = place_ptr.exit
            ants_lose()
...
class SlowThrower(ThrowerAnt):
    ...
    bees_slow = {}  # bee_instance:[slow_time_left, orig_action]

    def __init__(self, health=1):
        super().__init__(health)

    def is_bee_slowed(bee):
        """
        bee: a bee instance
        """
    ...
    def throw_at(self, target):
        ...
        def slow_action(bee):
            def foo(gamestate):
                if (
                    gamestate.time % 2 == 0  # time even take action
                    or not SlowThrower.is_bee_slowed(bee)  # not slowed
                ):
                    SlowThrower.bees_slow[bee][1](gamestate)
                SlowThrower.bees_slow[bee][0] -= 1
        ...
        if target in (bs := SlowThrower.bees_slow):
            bs[target][0] = 5
        else:
            bs[target] = [5, target.action]
            target.action = slow_action(target)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        place_ptr, distance = self.place, 0
        while (not place_ptr.is_hive) and place_ptr.entrance:  # all tiles front
            for insect in place_ptr.bees + [place_ptr.ant]:
                if insect == self or insect is None:
                    continue  # exclude itself
                ins_dis[insect] = distance
                if (
                    isinstance(insect, Ant)
                    and insect.is_container
                    and insect.ant_contained != self
                ):
                    ins_dis[insect.ant_contained] = distance
            place_ptr, distance = place_ptr.entrance, distance + 1
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.blocked():
            ...
            if self.is_scared_before and self.scare_countdown > 0:
                self.scare(1)
            else:
    ...
    def scare(self, length):
        ...
        if self.is_scared_before and self.scare_countdown < 0:
            return
        self.is_scared_before, self.scare_countdown = True, self.scare_countdown - 1
        i = 0
        while i < length and not self.place.entrance.is_hive:
